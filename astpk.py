...
def deploy(snapshot):
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{snapshot}")):
        ...
        tmp = get_tmp()
        os.system(f"btrfs sub set-default /.snapshots/rootfs/snapshot-{tmp} 2>&1> /dev/null") # Set default volume
        untmp()
        if "tmp0" in tmp:
            tmp = "tmp"
        else:
            tmp = "tmp0"
        etc = snapshot
        os.system(f"btrfs sub snap /.snapshots/rootfs/snapshot-{snapshot} /.snapshots/rootfs/snapshot-{tmp} 2>&1> /dev/null")
        os.system(f"btrfs sub snap /.snapshots/etc/etc-{snapshot} /.snapshots/etc/etc-{tmp} 2>&1> /dev/null")
        os.system(f"btrfs sub create /.snapshots/var/var-{tmp} 2>&1> /dev/null")
        os.system(f"cp --reflink=auto -r /.snapshots/var/var-{etc}/* /.snapshots/var/var-{tmp} 2>&1> /dev/null")
        os.system(f"btrfs sub snap /.snapshots/boot/boot-{snapshot} /.snapshots/boot/boot-{tmp} 2>&1> /dev/null")
        os.system(f"mkdir /.snapshots/rootfs/snapshot-{tmp}/etc 2>&1> /dev/null")
        os.system(f"rm -rf /.snapshots/rootfs/snapshot-{tmp}/var 2>&1> /dev/null")
        os.system(f"mkdir /.snapshots/rootfs/snapshot-{tmp}/boot 2>&1> /dev/null")
        os.system(f"cp --reflink=auto -r /.snapshots/etc/etc-{etc}/* /.snapshots/rootfs/snapshot-{tmp}/etc 2>&1> /dev/null")
        os.system(f"btrfs sub snap /var /.snapshots/rootfs/snapshot-{tmp}/var 2>&1> /dev/null")
        os.system(f"cp --reflink=auto -r /.snapshots/boot/boot-{etc}/* /.snapshots/rootfs/snapshot-{tmp}/boot 2>&1> /dev/null")
        os.system(f"echo '{snapshot}' > /.snapshots/rootfs/snapshot-{tmp}/usr/share/ast/snap")
        switchtmp()
        #os.system(f"rm -rf /var/lib/pacman/* 2>&1> /dev/null") # Clean pacman and systemd directories before copy
        os.system(f"rm -rf /var/lib/systemd/* 2>&1> /dev/null")
        os.system(f"rm -rf /.snapshots/rootfs/snapshot-{tmp}/var/lib/systemd/* 2>&1> /dev/null")
        #os.system(f"rm -rf /.snapshots/rootfs/snapshot-{tmp}/var/lib/pacman/* 2>&1> /dev/null")
        os.system(f"cp --reflink=auto -r /.snapshots/var/var-{etc}/* /.snapshots/rootfs/snapshot-{tmp}/var/ 2>&1> /dev/null")
        #os.system(f"cp --reflink=auto -r /.snapshots/var/var-{etc}/lib/pacman/* /var/lib/pacman/ 2>&1> /dev/null") # Copy pacman and systemd directories
        os.system(f"cp --reflink=auto -r /.snapshots/var/var-{etc}/lib/systemd/* /var/lib/systemd/ 2>&1> /dev/null")
        #os.system(f"cp --reflink=auto -r /.snapshots/var/var-{etc}/lib/pacman/* /.snapshots/rootfs/snapshot-{tmp}/var/lib/pacman/")
        os.system(f"cp --reflink=auto -r /.snapshots/var/var-{etc}/lib/systemd/* /.snapshots/rootfs/snapshot-{tmp}/var/lib/systemd/ 2>&1> /dev/null")
        os.system(f"btrfs sub set-default /.snapshots/rootfs/snapshot-{tmp}") # Set default volume
        #os.system(f"chattr -RV +i /.snapshots/rootfs/snapshot-{tmp}/usr 2>&1> /dev/null")
...
def extend_branch(snapshot, desc=""):
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{snapshot}")):
        ...
        i = findnew()
        os.system(f"btrfs sub snap -r /.snapshots/rootfs/snapshot-{snapshot} /.snapshots/rootfs/snapshot-{i} 2>&1> /dev/null")
        os.system(f"btrfs sub snap -r /.snapshots/etc/etc-{snapshot} /.snapshots/etc/etc-{i} 2>&1> /dev/null")
        os.system(f"btrfs sub snap -r /.snapshots/var/var-{snapshot} /.snapshots/var/var-{i} 2>&1> /dev/null")
        os.system(f"btrfs sub snap -r /.snapshots/boot/boot-{snapshot} /.snapshots/boot/boot-{i} 2>&1> /dev/null")
...
def clone_under(snapshot, branch):
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{snapshot}")) or (not(os.path.exists(f"/.snapshots/rootfs/snapshot-{branch}"))):
        ...
        i = findnew()
        os.system(f"btrfs sub snap -r /.snapshots/rootfs/snapshot-{branch} /.snapshots/rootfs/snapshot-{i} 2>&1> /dev/null")
        os.system(f"btrfs sub snap -r /.snapshots/etc/etc-{branch} /.snapshots/etc/etc-{i} 2>&1> /dev/null")
        os.system(f"btrfs sub snap -r /.snapshots/var/var-{branch} /.snapshots/var/var-{i} 2>&1> /dev/null")
        os.system(f"btrfs sub snap -r /.snapshots/boot/boot-{branch} /.snapshots/boot/boot-{i} 2>&1> /dev/null")
        add_node_to_parent(fstree,snapshot,i)
        write_tree(fstree)
        desc = str(f"clone of {snapshot}")
        write_desc(i, desc)
...
def sync_tree(tree,treename,forceOffline):
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{treename}")):
        ...
        while True:
            ...
            order.remove(order[0])
            prepare(sarg)
            #os.system(f"cp --reflink=auto -r /.snapshots/var/var-{arg}/lib/pacman/local/* /.snapshots/var/var-chr{sarg}/lib/pacman/local/ 2>&1> /dev/null")
            os.system(f"cp --reflink=auto -r /.snapshots/var/var-{arg}/lib/systemd/* /.snapshots/var/var-chr{sarg}/lib/systemd/ 2>&1> /dev/null")
            #os.system(f"cp --reflink=auto -r /.snapshots/var/var-{arg}/lib/pacman/local/* /.snapshots/rootfs/snapshot-chr{sarg}/var/lib/pacman/local/ 2>&1> /dev/null")
            os.system(f"cp --reflink=auto -r /.snapshots/var/var-{arg}/lib/systemd/* /.snapshots/rootfs/snapshot-chr{sarg}/var/lib/systemd/ 2>&1> /dev/null")
            os.system(f"cp --reflink=auto -n -r /.snapshots/rootfs/snapshot-{arg}/* /.snapshots/rootfs/snapshot-chr{sarg}/ 2>&1> /dev/null")
            posttrans(sarg)
...
def new_snapshot(desc=""):
    i = findnew()
    os.system(f"btrfs sub snap -r /.snapshots/rootfs/snapshot-0 /.snapshots/rootfs/snapshot-{i} 2>&1> /dev/null")
    os.system(f"btrfs sub snap -r /.snapshots/etc/etc-0 /.snapshots/etc/etc-{i} 2>&1> /dev/null")
    os.system(f"btrfs sub snap -r /.snapshots/boot/boot-0 /.snapshots/boot/boot-{i} 2>&1> /dev/null")
    os.system(f"btrfs sub snap -r /.snapshots/var/var-0 /.snapshots/var/var-{i} 2>&1> /dev/null")
    append_base_tree(fstree,i)
    write_tree(fstree)
    if desc: write_desc(i, desc)
...
def update_etc():
    tmp = get_tmp()
    snapshot = get_snapshot()
    os.system(f"btrfs sub del /.snapshots/etc/etc-{snapshot} 2>&1> /dev/null")
    os.system(f"btrfs sub snap -r /.snapshots/etc/etc-{tmp} /.snapshots/etc/etc-{snapshot} 2>&1> /dev/null")
...
def unchr(snapshot):
    os.system(f"btrfs sub del /.snapshots/etc/etc-chr{snapshot} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/var/var-chr{snapshot} 2>&1> /dev/null")
    os.system(f"rm -rf /.snapshots/var/var-chr{snapshot} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/boot/boot-chr{snapshot} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-chr{snapshot}/* 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-chr{snapshot} 2>&1> /dev/null")
...
def untmp():
    ...
    if "tmp0" in tmp:
        ...
        tmp = "tmp0"
    os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-{tmp}/* 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-{tmp} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/etc/etc-{tmp} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/var/var-{tmp} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/boot/boot-{tmp} 2>&1> /dev/null")
...
def live_install(pkg):
    ...
    part = get_part()
    #os.system(f"chattr -RV -i /.snapshots/rootfs/snapshot-{tmp}/usr 2>&1> /dev/null")
    os.system(f"mount --bind /.snapshots/rootfs/snapshot-{tmp} /.snapshots/rootfs/snapshot-{tmp} 2>&1> /dev/null")
    os.system(f"mount --bind /home /.snapshots/rootfs/snapshot-{tmp}/home 2>&1> /dev/null")
    os.system(f"mount --bind /var /.snapshots/rootfs/snapshot-{tmp}/var 2>&1> /dev/null")
    os.system(f"mount --bind /etc /.snapshots/rootfs/snapshot-{tmp}/etc 2>&1> /dev/null")
    os.system(f"mount --bind /tmp /.snapshots/rootfs/snapshot-{tmp}/tmp 2>&1> /dev/null")
    os.system(f"arch-chroot /.snapshots/rootfs/snapshot-{tmp} pacman -S  --overwrite \\* --noconfirm {pkg}")
    os.system(f"umount /.snapshots/rootfs/snapshot-{tmp}/* 2>&1> /dev/null")
    os.system(f"umount /.snapshots/rootfs/snapshot-{tmp} 2>&1> /dev/null")
    #os.system(f"chattr -RV +i /.snapshots/rootfs/snapshot-{tmp}/usr 2>&1> /dev/null")
...
def delete(snapshot):
    ...
    if not (os.path.exists(f"/.snapshots/rootfs/snapshot-{snapshot}")):
        ...
        os.system(f"btrfs sub del /.snapshots/boot/boot-{snapshot} 2>&1> /dev/null")
        os.system(f"btrfs sub del /.snapshots/etc/etc-{snapshot} 2>&1> /dev/null")
        os.system(f"btrfs sub del /.snapshots/var/var-{snapshot} 2>&1> /dev/null")
        os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-{snapshot} 2>&1> /dev/null")
        for child in children: # This deletes the node itself along with it's children
            os.system(f"btrfs sub del /.snapshots/boot/boot-{child} 2>&1> /dev/null")
            os.system(f"btrfs sub del /.snapshots/etc/etc-{child} 2>&1> /dev/null")
            os.system(f"btrfs sub del /.snapshots/var/var-{child} 2>&1> /dev/null")
            os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-{child} 2>&1> /dev/null")
...
def prepare(snapshot):
    ...
    part = get_part()
    etc = snapshot
    os.system(f"btrfs sub snap /.snapshots/rootfs/snapshot-{snapshot} /.snapshots/rootfs/snapshot-chr{snapshot} 2>&1> /dev/null")
    os.system(f"btrfs sub snap /.snapshots/etc/etc-{snapshot} /.snapshots/etc/etc-chr{snapshot} 2>&1> /dev/null")
    os.system(f"mkdir -p /.snapshots/var/var-chr{snapshot} 2>&1> /dev/null")
    os.system(f"mount --bind /.snapshots/rootfs/snapshot-chr{snapshot} /.snapshots/rootfs/snapshot-chr{snapshot} 2>&1> /dev/null") # Pacman gets weird when chroot directory is not a mountpoint, so this mount is necessary
    os.system(f"mount --bind /var /.snapshots/rootfs/snapshot-chr{snapshot}/var 2>&1> /dev/null")
    os.system(f"mount --rbind /dev /.snapshots/rootfs/snapshot-chr{snapshot}/dev 2>&1> /dev/null")
    os.system(f"mount --rbind /sys /.snapshots/rootfs/snapshot-chr{snapshot}/sys 2>&1> /dev/null")
    os.system(f"mount --rbind /tmp /.snapshots/rootfs/snapshot-chr{snapshot}/tmp 2>&1> /dev/null")
    os.system(f"mount --rbind /proc /.snapshots/rootfs/snapshot-chr{snapshot}/proc 2>&1> /dev/null")
    #os.system(f"chmod 0755 /.snapshots/rootfs/snapshot-chr/var 2>&1> /dev/null") # For some reason the permission needs to be set here
    os.system(f"btrfs sub snap /.snapshots/boot/boot-{snapshot} /.snapshots/boot/boot-chr{snapshot} 2>&1> /dev/null")
    os.system(f"cp -r --reflink=auto /.snapshots/etc/etc-chr{snapshot}/* /.snapshots/rootfs/snapshot-chr{snapshot}/etc 2>&1> /dev/null")
    os.system(f"cp -r --reflink=auto /.snapshots/boot/boot-chr{snapshot}/* /.snapshots/rootfs/snapshot-chr{snapshot}/boot 2>&1> /dev/null")
    #os.system(f"rm -rf /.snapshots/rootfs/snapshot-chr{snapshot}/var/lib/pacman/* 2>&1> /dev/null")
    os.system(f"rm -rf /.snapshots/rootfs/snapshot-chr{snapshot}/var/lib/systemd/* 2>&1> /dev/null")
    #os.system(f"cp -r --reflink=auto /.snapshots/var/var-{snapshot}/lib/pacman/* /.snapshots/rootfs/snapshot-chr{snapshot}/var/lib/pacman/ 2>&1> /dev/null")
    os.system(f"cp -r --reflink=auto /.snapshots/var/var-{snapshot}/lib/systemd/* /.snapshots/rootfs/snapshot-chr{snapshot}/var/lib/systemd/ 2>&1> /dev/null")
    os.system(f"mount --bind /home /.snapshots/rootfs/snapshot-chr{snapshot}/home 2>&1> /dev/null")
    os.system(f"mount --rbind /run /.snapshots/rootfs/snapshot-chr{snapshot}/run 2>&1> /dev/null")
    ...
    os.system(f"mount --bind /etc/resolv.conf /.snapshots/rootfs/snapshot-chr{snapshot}/etc/resolv.conf 2>&1> /dev/null")

# Post transaction function, copy from chroot dirs back to read only image dir
def posttrans(snapshot):
    etc = snapshot
    tmp = get_tmp()
    os.system(f"umount /.snapshots/rootfs/snapshot-chr{snapshot} 2>&1> /dev/null")
    os.system(f"umount /.snapshots/rootfs/snapshot-chr{snapshot}/etc/resolv.conf 2>&1> /dev/null")
    os.system(f"umount /.snapshots/rootfs/snapshot-chr{snapshot}/home 2>&1> /dev/null")
    os.system(f"umount /.snapshots/rootfs/snapshot-chr{snapshot}/run 2>&1> /dev/null")
    os.system(f"umount /.snapshots/rootfs/snapshot-chr{snapshot}/dev 2>&1> /dev/null")
    os.system(f"umount /.snapshots/rootfs/snapshot-chr{snapshot}/sys 2>&1> /dev/null")
    os.system(f"umount /.snapshots/rootfs/snapshot-chr{snapshot}/proc 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-{snapshot} 2>&1> /dev/null")
    os.system(f"rm -rf /.snapshots/etc/etc-chr{snapshot}/* 2>&1> /dev/null")
    os.system(f"cp -r --reflink=auto /.snapshots/rootfs/snapshot-chr{snapshot}/etc/* /.snapshots/etc/etc-chr{snapshot} 2>&1> /dev/null")
    os.system(f"rm -rf /.snapshots/var/var-chr{snapshot}/* 2>&1> /dev/null")
    os.system(f"mkdir -p /.snapshots/var/var-chr{snapshot}/lib/systemd 2>&1> /dev/null")
    #os.system(f"mkdir -p /.snapshots/var/var-chr{snapshot}/lib/pacman 2>&1> /dev/null")
    os.system(f"cp -r --reflink=auto /.snapshots/rootfs/snapshot-chr{snapshot}/var/lib/systemd/* /.snapshots/var/var-chr{snapshot}/lib/systemd 2>&1> /dev/null")
    #os.system(f"cp -r --reflink=auto /.snapshots/rootfs/snapshot-chr{snapshot}/var/lib/pacman/* /.snapshots/var/var-chr{snapshot}/lib/pacman 2>&1> /dev/null")
    os.system(f"cp -r -n --reflink=auto /.snapshots/rootfs/snapshot-chr{snapshot}/var/cache/pacman/pkg/* /var/cache/pacman/pkg/ 2>&1> /dev/null")
    os.system(f"rm -rf /.snapshots/boot/boot-chr{snapshot}/* 2>&1> /dev/null")
    os.system(f"cp -r --reflink=auto /.snapshots/rootfs/snapshot-chr{snapshot}/boot/* /.snapshots/boot/boot-chr{snapshot} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/etc/etc-{etc} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/var/var-{etc} 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/boot/boot-{etc} 2>&1> /dev/null")
    os.system(f"btrfs sub snap -r /.snapshots/etc/etc-chr{snapshot} /.snapshots/etc/etc-{etc} 2>&1> /dev/null")
    os.system(f"btrfs sub create /.snapshots/var/var-{etc} 2>&1> /dev/null")
    os.system(f"mkdir -p /.snapshots/var/var-{etc}/lib/systemd 2>&1> /dev/null")
    #os.system(f"mkdir -p /.snapshots/var/var-{etc}/lib/pacman 2>&1> /dev/null")
    os.system(f"cp --reflink=auto -r /.snapshots/var/var-chr{snapshot}/lib/systemd/* /.snapshots/var/var-{etc}/lib/systemd 2>&1> /dev/null")
    #os.system(f"cp --reflink=auto -r /.snapshots/rootfs/snapshot-chr{snapshot}/.snapshots/ast/fstree /.snapshots/ast/ 2>&1> /dev/null")
    #os.system(f"cp --reflink=auto -r /.snapshots/var/var-chr{snapshot}/lib/pacman/* /.snapshots/var/var-{etc}/lib/pacman 2>&1> /dev/null")
    #os.system(f"rm -rf /var/lib/pacman/* 2>&1> /dev/null")
    #os.system(f"cp --reflink=auto -r /.snapshots/rootfs/snapshot-{tmp}/var/lib/pacman/* /var/lib/pacman 2>&1> /dev/null")
    os.system(f"rm -rf /var/lib/systemd/* 2>&1> /dev/null")
    os.system(f"cp --reflink=auto -r /.snapshots/rootfs/snapshot-{tmp}/var/lib/systemd/* /var/lib/systemd 2>&1> /dev/null")
    #os.system(f"cp --reflink=auto -r -n /.snapshots/rootfs/snapshot-chr{snapshot}/var/lib/* /var/lib/ 2>&1> /dev/null")
    #os.system(f"cp --reflink=auto -r -n /.snapshots/rootfs/snapshot-chr{snapshot}/var/games/* /var/games/ 2>&1> /dev/null")
    os.system(f"btrfs sub snap -r /.snapshots/rootfs/snapshot-chr{snapshot} /.snapshots/rootfs/snapshot-{snapshot} 2>&1> /dev/null")
#    os.system(f"btrfs sub snap -r /.snapshots/var/var-chr{snapshot} /.snapshots/var/var-{etc} 2>&1> /dev/null")
    os.system(f"btrfs sub snap -r /.snapshots/boot/boot-chr{snapshot} /.snapshots/boot/boot-{etc} 2>&1> /dev/null")
    unchr(snapshot)
...
def switchtmp():
    ...
    os.system(f"mkdir -p /etc/mnt/boot 2>&1> /dev/null")
...
#        os.system("sed -i 's,@.snapshots/var/var-tmp,@.snapshots/var/var-tmp0,g' /.snapshots/rootfs/snapshot-tmp0/etc/fstab")
    ...
    grubconf.write("### END /etc/grub.d/41_custom ###")
    grubconf.close()
    os.system("umount /etc/mnt/boot 2>&1> /dev/null")

# Clear all temporary snapshots
def tmpclear():
    os.system(f"btrfs sub del /.snapshots/etc/etc-chr* 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/var/var-chr* 2>&1> /dev/null")
    os.system(f"rm -rf /.snapshots/var/var-chr* 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/boot/boot-chr* 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-chr*/* 2>&1> /dev/null")
    os.system(f"btrfs sub del /.snapshots/rootfs/snapshot-chr* 2>&1> /dev/null")
