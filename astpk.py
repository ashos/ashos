...
def aur_setup_live(snap):
    ...
    excode = int(os.system(f"chroot /.snapshots/rootfs/snapshot-{snap} pacman -S --noconfirm --needed sudo git base-devel"))
    if excode:
        return excode
    ...
    excode = int(os.system(f"chroot /.snapshots/rootfs/snapshot-{snap} su aur -c 'rm -rf /home/aur/paru-bin && cd /home/aur && git clone https://aur.archlinux.org/paru-bin.git' >/dev/null 2>&1"))
    if excode:
        os.system(f"umount /.snapshots/rootfs/snapshot-{tmp}/* >/dev/null 2>&1")
        os.system(f"umount /.snapshots/rootfs/snapshot-{tmp} >/dev/null 2>&1")
        print("F: failed to download paru-bin")
        return excode
    excode = int(os.system(f"chroot /.snapshots/rootfs/snapshot-{snap} su aur -c 'cd /home/aur/paru-bin && makepkg -si'"))
    if excode:
        os.system(f"umount /.snapshots/rootfs/snapshot-{tmp}/* >/dev/null 2>&1")
        os.system(f"umount /.snapshots/rootfs/snapshot-{tmp} >/dev/null 2>&1")
        print("F: failed installing paru-bin")
...
def main(args):
    ...
    if isChroot == True and ("--chroot" not in args):
        ...
        if live and not excode: # only perform the live_install if the first install was successful
            options = get_persnap_options(csnapshot) # Check if AUR is enabled in target snapshot
            if options["aur"] == "True":
                aur = True
            else:
                aur = False
