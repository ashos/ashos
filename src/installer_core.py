...
def grub_ash(v):
  ...
  # grub-install rewrites default core.img, so run grub-mkimage AFTER!
    ...
    if is_efi:
        if is_external_boot: ### REVIEW
            os.system(f"efibootmgr -c -d {external_boot} -p 1 -L {distro_name} -l '\\EFI\\{distro}\\grubx64.efi'")
...
###    os.system("sudo ln -srf /mnt/.snapshots/ash/tmp /mnt/tmp")
  ...
  # Update fstab
    for mntdir in mntdirs:
        os.system(f"echo 'UUID=\"{to_uuid(external_boot if is_external_boot and mntdir == 'boot' else os_root)}\" /{mntdir} btrfs subvol=@{mntdir}{distro_suffix},compress=zstd,noatime{'' if mntdir else ',ro'} 0 0' | sudo tee -a /mnt/etc/fstab") # ro only for / entry ### complex but one-liner
    if is_efi:
