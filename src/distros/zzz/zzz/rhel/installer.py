...
os.system("echo 'LANG=en_US.UTF-8' | sudo tee /mnt/etc/locale.conf") ### REVIEW single quotes redundant
...
os.system('grep -qxF "[#]?GRUB_ENABLE_BLSCFG.*" /tmp/reza/grub.cfg && sudo sed -i "s/[#]?GRUB_ENABLE_BLSCFG.*$/GRUB_ENABLE_BLSCFG=true/" /tmp/reza/grub.cfg || \
           echo GRUB_ENABLE_BLSCFG="false" | sudo tee -a /tmp/reza/grub.cfg')

grep -qxF "#?GRUB_ENABLE_BLSCFG.*" /tmp/reza/grub.cfg && sed -i "s/#?GRUB_ENABLE_BLSCFG.*$/GRUB_ENABLE_BLSCFG=true/" /tmp/reza/grub.cfg || \
           echo GRUB_ENABLE_BLSCFG="false" | tee -a /tmp/reza/grub.cfg

grep -qxF "GRUB_ENABLE_BLSCFG" /tmp/reza/grub.cfg && sed -i "s/#?GRUB_ENABLE_BLSCFG.*$/GRUB_ENABLE_BLSCFG=true/" /tmp/reza/grub.cfg

sed -i "s/^GRUB_ENABLE_BLSCFG.*$/GRUB_ENABLE_BLSCFG=true/" /tmp/reza/grub.cfg || echo GRUB_ENABLE_BLSCFG="false" >> /tmp/reza/grub.cfg

awk -i inplace '{gsub("^#?GRUB_ENABLE_BLSCFG.*$","GRUB_ENABLE_BLSCFG=true"); print;}' /tmp/reza/grub.cfg || echo GRUB_ENABLE_BLSCFG="false" >> /tmp/reza/grub.cfg
