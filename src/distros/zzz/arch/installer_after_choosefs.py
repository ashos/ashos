...
#from src.installer_core import is_luks, ash_chroot, clear, deploy_base_snapshot, deploy_to_common, get_hostname, get_item_from_path, grub_ash, is_efi, post_bootstrap, pre_bootstrap, unmounts, fs, refind_ash, os_root
...
def initram_update():
    if is_luks:
        ...
        os.system(f"sudo cryptsetup luksAddKey {os_root} /mnt/etc/crypto_keyfile.bin")
    ...
    if is_lvm:
        os.system("sed -i -e '/^HOOKS/{ s/modconf block/modconf block lvm2/g }' /mnt/etc/mkinitcpio.conf")
    ...
    if is_luks or is_lvm: ### REVIEW so that initram doesn't get built redundantly
        os.system(f"sudo chroot /mnt sudo mkinitcpio -p linux{KERNEL}")

#   1. Define variables
KERNEL = "" # options: https://wiki.archlinux.org/title/kernel
packages = f"base linux{KERNEL} sudo python3 python-anytree dhcpcd networkmanager nano" # linux-firmware os-prober bash tmux arch-install-scripts
super_group = "wheel"
...
bootloader = "grub"
if not is_lvm:
    packages = f"{packages} btrfs-progs"
else:
    if fs == "ext4":
        packages = f"{packages} lvm2 e2fsprogs gcc gcc-libs snapper" # libstdc++ required
    elif fs == "xfs":
        packages = f"{packages} lvm2 xfsprogs gcc gcc-libs snapper" # libstdc++ required
if bootloader == "grub":
    packages = f"{packages} grub"
